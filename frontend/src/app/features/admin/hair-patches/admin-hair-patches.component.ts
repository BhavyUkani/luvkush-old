  ...
  template: `
    <div class="page">
      ...
      <div class="filter-bar">
        ...
        <button class="btn-primary" (click)="createPatch()">
          ...
          <tbody>
            @for (item of items(); track item.id) {
              <tr [class.row-selected]="selectedIds().has(item.id)" [class.row-draft]="item.status === 'draft'">
                ...
                <td>
                  @if (item.size_info) { <span class="cat-chip">{{ item.size_info }}</span> } @else { <span class="muted">—</span> }
                </td>
                <td>
                ...
                <td class="ta-right">
                  <div class="action-btns">
                    <button class="icon-btn icon-btn--edit" (click)="editPatch(item.id)" title="Edit" aria-label="Edit">
            ...
            @if (!items().length) {
              <tr><td colspan="6" class="empty-cell">
                ...
                <div>No hair patches yet. Click "Add Patch" to create one.</div>
              </td></tr>
            }
          </tbody>
        </table>
