...
export class PaymentService {
   ...
   */
  private async computeRequiredAdvance(orderId: number): Promise<number> {
    ...
    const order = await db.queryOne<any>('SELECT total_amount FROM orders WHERE id = ?', [orderId]);
    return Math.min(required, Number(order?.total_amount || 0));
  }

  async createRazorpayOrder(orderId: number, userId: number, isPartial: boolean) {
    ...
    if (isPartial) {
      ...
      if (chargeAmount <= 0) throw new AppError('This order does not support a partial payment', 400);
    }
    ...
    await db.query(
      'UPDATE orders SET razorpay_order_id = ?, pending_charge_amount = ?, updated_at = NOW() WHERE id = ?',
      [rzpOrder.id, chargeAmount, orderId]
    );
  ...
  async verifyAndCapture(data: {
    ...
    const order = await db.queryOne<any>(
      'SELECT id, razorpay_order_id, payment_status, total_amount, pending_charge_amount FROM orders WHERE id = ? AND user_id = ?',
      [order_id, user_id]
    );
    if (!order) throw new AppError('Order not found', 404);
  ...
  async handleWebhook(payload: any, signature: string | undefined, rawBody: Buffer | undefined) {
    ...
    if (!signature || !rawBody) {
      throw new AppError('Invalid webhook request', 400);
    }
    ...
    if (event === 'payment.captured' && paymentEntity) {
      ...
      const order = await db.queryOne<any>(
        'SELECT id, user_id, payment_status, pending_charge_amount, total_amount FROM orders WHERE razorpay_order_id = ?', [rzpOrderId]
      );
  ...
  async initiateRefund(orderId: number, amount: number | undefined, adminUserId?: number | null) {
    const order = await db.queryOne<any>(
      'SELECT razorpay_payment_id, total_amount, payment_status FROM orders WHERE id = ?', [orderId]
    );
    if (!order?.razorpay_payment_id) throw new AppError('No payment found for this order', 400);

    const ledger = await db.queryOne<any>(`
      SELECT
    ...
    if (refundable <= 0) throw new AppError('This order has already been fully refunded', 400);
    ...
    if (requested > refundable + 0.01) {
      throw new AppError(`Refund amount cannot exceed the refundable balance of ₹${refundable.toFixed(2)}`, 400);
    }
