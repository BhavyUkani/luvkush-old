...
<body style="margin:0;padding:0;background:#F2EDE3;font-family:'Georgia',serif">
  <div style="max-width:600px;margin:0 auto;background:#ffffff">
    ...
    async sendOrderConfirmation(email, name, order) {
      ...
      <a href="${config_1.config.frontendUrl}/account/orders"
    ...
    async sendContactAcknowledgement(email, name) {
        const content = `
      <h2 style="font-size:28px;font-weight:300;color:#6B3A2A;margin:0 0 16px">
        Thank You, ${name}
      </h2>
    ...
    async send(opts) {
        try {
            await this.transporter.sendMail({
                from: `"${config_1.config.smtp.fromName}" <${config_1.config.smtp.from}>`,
                to: opts.to,
                subject: opts.subject,
                html: opts.html
            });
            logger_1.logger.info(`Email sent to ${opts.to}: ${opts.subject}`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to send email to ${opts.to}:`, error);
            throw error;
        }
    }
}
