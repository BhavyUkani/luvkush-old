...
async function seedDemoData() {
    logger_1.logger.info('Starting full demo data seed...');
    try {
        // 0. Ensure a demo user exists for reviews
        let demoUser = await database_1.db.queryOne('SELECT id FROM users LIMIT 1');
        if (!demoUser) {
            const res = await database_1.db.query(`INSERT INTO users (first_name, last_name, email, password_hash, role, status)
        ...
        for (const cat of categoriesData) {
            const existing = await database_1.db.queryOne('SELECT id FROM categories WHERE slug = ?', [cat.slug]);
            if (existing) {
                await database_1.db.query(`UPDATE categories SET name = ?, description = ?, image_url = ?, is_featured = ?, status = 'active', display_order = ? WHERE id = ?`, [cat.name, cat.description, cat.image_url, cat.is_featured, cat.display_order, existing.id]);
                categoryMap.set(cat.slug, existing.id);
            }
            else {
                const res = await database_1.db.query(`INSERT INTO categories (name, slug, description, image_url, display_order, is_featured, status) VALUES (?, ?, ?, ?, ?, ?, 'active')`, [cat.name, cat.slug, cat.description, cat.image_url, cat.display_order, cat.is_featured]);
                categoryMap.set(cat.slug, res.insertId);
            }
        }
        logger_1.logger.info(`Seeded ${categoryMap.size} categories.`);
        // 2. Seed Products
        const productsData = [
        ...
        for (const p of productsData) {
            ...
            if (!categoryId)
                continue;
            const existing = await database_1.db.queryOne('SELECT id FROM products WHERE slug = ?', [p.slug]);
            let productId;
            if (existing) {
                productId = existing.id;
                await database_1.db.query(`UPDATE products SET
          ...
          WHERE id = ?`, [
                ...
                const res = await database_1.db.query(`INSERT INTO products (
            ...
            if (p.variants && p.variants.length > 0) {
                ...
                for (const v of p.variants) {
                    await database_1.db.query(`INSERT INTO product_variants (product_id, name, value, price_modifier, stock_quantity, status) VALUES (?, ?, ?, ?, ?, 'active')`, [productId, v.name, v.value, v.price_modifier, v.stock_quantity]);
                }
            }
            // Add Reviews
            if (p.reviews && p.reviews.length > 0) {
                await database_1.db.query('DELETE FROM reviews WHERE product_id = ?', [productId]);
                for (const rev of p.reviews) {
                    await database_1.db.query(`INSERT INTO reviews (product_id, user_id, rating, title, body, is_verified_purchase, status) VALUES (?, ?, ?, ?, ?, 1, 'approved')`, [productId, userId, rev.rating, rev.title, rev.body]);
                }
            }
        }
        logger_1.logger.info(`Seeded ${productsData.length} products with reviews & variants.`);
        ...
        for (const hs of hairSolutionsData) {
            const existing = await database_1.db.queryOne('SELECT id FROM hair_solutions WHERE slug = ?', [hs.slug]);
            if (existing) {
                await database_1.db.query(`UPDATE hair_solutions SET
          ...
          WHERE id = ?`, [
                ...
                await database_1.db.query(`INSERT INTO hair_solutions (
        ...
        logger_1.logger.info(`Seeded ${hairSolutionsData.length} hair solutions (wigs & patches).`);
        logger_1.logger.info('Full demo data seed completed successfully!');
        process.exit(0);
    }
    catch (error) {
        logger_1.logger.error('Error seeding demo data:', error);
        process.exit(1);
    }
}
seedDemoData();
