...
if (exports.config.nodeEnv === 'production') {
    const problems = [];
    if (DEFAULT_SECRETS.includes(exports.config.jwt.accessSecret))
        problems.push('JWT_SECRET');
    if (DEFAULT_SECRETS.includes(exports.config.jwt.refreshSecret))
        problems.push('JWT_REFRESH_SECRET');
    if (!exports.config.razorpay.webhookSecret)
        problems.push('RAZORPAY_WEBHOOK_SECRET');
    if (problems.length) {
        throw new Error(`Refusing to start in production with insecure/missing configuration: ${problems.join(', ')}. ` +
