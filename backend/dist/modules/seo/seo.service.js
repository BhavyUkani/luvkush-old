...
class SeoService {
    async getMetaForPath(urlPath) {
        if (urlPath.startsWith('/products/')) {
            const slug = urlPath.replace('/products/', '');
            const product = await database_1.db.queryOne('SELECT name, seo_title, seo_description, seo_keywords, primary_image FROM products WHERE slug = ? AND status = "active"', [slug]);
            if (product) {
                return {
        ...
        if (urlPath.startsWith('/collections/')) {
            ...
            // maintains.
            const category = await database_1.db.queryOne("SELECT name, meta_title, meta_description FROM categories WHERE slug = ? AND status = 'active'", [slug]);
            if (category) {
                return {
