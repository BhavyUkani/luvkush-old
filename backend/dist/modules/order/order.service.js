...
function isBackwardTransition(from, to) {
    if (from === to || TERMINAL_STATUSES.includes(to))
        return false;
...
class OrderService {
    async createFromCart(userId, data) {
        ...
        for (const item of cart.items) {
            if (item.product_status !== 'active') {
                throw new error_middleware_1.AppError(`"${item.name}" is no longer available`, 400);
            }
        ...
        const shippingMethod = data.shipping_method === 'express' ? 'express' : 'standard';
        const shippingCost = (0, business_rules_1.calculateShippingCost)(subtotal - discount, shippingMethod, freeShipping);
        const taxAmount = (0, business_rules_1.calculateTax)(subtotal - discount);
        ...
        for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            ...
            try {
                const createdOrderId = await database_1.db.transaction(async (conn) => {
          ...
          `, [
            ...
            `, [orderId, item.product_id, item.variant_id || null, item.name, item.variant_name || null, item.quantity, unitPrice, item.mrp || unitPrice, lineTotal, item.primary_image || null]);
                        if (item.variant_id) {
                            const [result] = await conn.execute('UPDATE product_variants SET stock_quantity = stock_quantity - ? WHERE id = ? AND stock_quantity >= ?', [item.quantity, item.variant_id, item.quantity]);
                            if (result.affectedRows === 0) {
                                throw new error_middleware_1.AppError(`"${item.name}" no longer has enough stock`, 409);
                            }
                        }
                        else {
                            const [result] = await conn.execute('UPDATE products SET stock_quantity = stock_quantity - ?, sales_count = sales_count + ? WHERE id = ? AND stock_quantity >= ?', [item.quantity, item.quantity, item.product_id, item.quantity]);
                            if (result.affectedRows === 0) {
                                throw new error_middleware_1.AppError(`"${item.name}" no longer has enough stock`, 409);
                            }
                        }
                    ...
                    if (couponCode) {
                        const [couponResult] = await conn.execute('UPDATE coupons SET usage_count = usage_count + 1 WHERE code = ? AND (usage_limit IS NULL OR usage_count < usage_limit)', [couponCode]);
                        if (couponResult.affectedRows === 0) {
                            throw new error_middleware_1.AppError('This coupon has just reached its usage limit', 409);
                        }
                    }
                    ...
                    if (data.payment_method !== 'razorpay' && data.payment_method !== 'online') {
                        await conn.execute('DELETE FROM cart_items WHERE cart_id = ?', [cart.cart_id]);
                    }
                    return orderId;
                });
    ...
    async getById(orderId, userId) {
        ...
        const order = await database_1.db.queryOne(`
        ...
        if (order.tracking_number && !['delivered', 'cancelled', 'returned', 'refunded'].includes(order.status)) {
            ...
            if (timeDiff > 15 * 60 * 1000) {
                try {
                    ...
                    const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error('Shiprocket tracking lookup timed out')), 5000));
                    ...
                    if (trackData && trackData.current_status) {
                        ...
                        if (synced) {
                            ...
                            const updated = await database_1.db.queryOne('SELECT status, updated_at FROM orders WHERE id = ?', [orderId]);
                            if (updated) {
                                order.status = updated.status;
                                order.updated_at = updated.updated_at;
                                const os = await database_1.db.queryOne('SELECT name, color FROM order_statuses WHERE slug = ?', [updated.status]);
                                if (os) {
                    ...
                    logger_1.logger.error('Failed to auto-sync order status with Shiprocket:', err);
                }
            }
        }
        order.items = await database_1.db.query(`
    ...
    async getByOrderNumber(orderNumber, userId) {
        const whereExtra = userId ? 'AND o.user_id = ?' : '';
        const params = userId ? [orderNumber, userId] : [orderNumber];
        const order = await database_1.db.queryOne(`SELECT o.id FROM orders o WHERE o.order_number = ? ${whereExtra}`, params);
        if (!order)
            return null;
    ...
    `;
        const paginated = await database_1.db.paginate(sql, [userId], page, limit);
    ...
    async getStatusCounts() {
        const rows = await database_1.db.query('SELECT status, COUNT(*) as cnt FROM orders GROUP BY status');
        const counts = {};
        for (const r of rows)
            counts[r.status] = Number(r.cnt);
        return counts;
    }
    async adminGetAll(page = 1, limit = 20, filters) {
        const conditions = [];
        const params = [];
        if (filters.status) {
    ...
    async updateStatus(orderId, status, note, date, changedBy) {
        const statuses = await database_1.db.query('SELECT slug FROM order_statuses');
        const validStatuses = statuses.map(s => s.slug);
        if (!validStatuses.includes(status))
            throw new error_middleware_1.AppError('Invalid order status', 400);
        ...
        await database_1.db.transaction(async (conn) => {
            const [rows] = await conn.execute('SELECT id, status, coupon_code FROM orders WHERE id = ? FOR UPDATE', [orderId]);
            const order = rows[0];
            if (!order)
                throw new error_middleware_1.AppError('Order not found', 404);
            if (TERMINAL_STATUSES.includes(order.status) && order.status !== status) {
                throw new error_middleware_1.AppError(`Order is already ${order.status} and cannot be moved to ${status}`, 400);
            }
            if (isBackwardTransition(order.status, status)) {
                throw new error_middleware_1.AppError(`Cannot move an order from "${order.status}" back to "${status}"`, 400);
            }
            ...
            await conn.execute('INSERT INTO order_status_history (order_id, status, note, changed_by, created_at) VALUES (?, ?, ?, ?, ?)', [orderId, status, note || null, changedBy || null, historyDate]);
            ...
            if (status === 'cancelled' && previousStatus !== 'cancelled' && order.coupon_code) {
                await conn.execute('UPDATE coupons SET usage_count = GREATEST(0, usage_count - 1) WHERE code = ? AND usage_count > 0', [order.coupon_code]);
            }
        });
    }
    async getStatusHistory(orderId) {
        try {
            return await database_1.db.query(`
      ...
      `, [orderId]);
            ...
            logger_1.logger.error('[OrderService] Failed to load status history:', err);
            return [];
        }
    }
    async updateTracking(orderId, data) {
        const order = await database_1.db.queryOne('SELECT status FROM orders WHERE id = ?', [orderId]);
        if (!order)
            throw new error_middleware_1.AppError('Order not found', 404);
        ...
        await database_1.db.query('UPDATE orders SET tracking_number = ?, tracking_url = ?, status = ?, updated_at = NOW() WHERE id = ?', [data.tracking_number, data.tracking_url || null, nextStatus, orderId]);
        if (nextStatus !== order.status) {
            try {
                await database_1.db.query("INSERT INTO order_status_history (order_id, status, note, created_at) VALUES (?, ?, 'Tracking number attached', NOW())", [orderId, nextStatus]);
            }
            catch (err) {
                logger_1.logger.error('[OrderService] Failed to save status history for tracking update:', err);
            }
        }
    }
    async cancelOrder(orderId, userId, reason) {
        const order = await database_1.db.queryOne('SELECT id, status, user_id FROM orders WHERE id = ? AND user_id = ?', [orderId, userId]);
        if (!order)
            throw new error_middleware_1.AppError('Order not found', 404);
    ...
    async abortOnlineOrder(orderId, userId) {
        await database_1.db.transaction(async (conn) => {
            ...
            const [rows] = await conn.execute('SELECT id, status, payment_status, payment_method FROM orders WHERE id = ? AND user_id = ? FOR UPDATE', [orderId, userId]);
            const order = rows[0];
            if (!order)
                throw new error_middleware_1.AppError('Order not found', 404);
            if (order.payment_status === 'paid')
                throw new error_middleware_1.AppError('Cannot abort a paid order', 400);
            if (!['razorpay', 'online'].includes(order.payment_method)) {
                throw new error_middleware_1.AppError('Only online payments can be aborted this way', 400);
            }
    ...
    async releaseAbandonedOnlineOrders(olderThanMinutes) {
        const candidates = await database_1.db.query(`SELECT id FROM orders
       WHERE payment_method IN ('razorpay', 'online')
         AND status = 'pending' AND payment_status = 'pending'
         AND updated_at < DATE_SUB(NOW(), INTERVAL ? MINUTE)`, [olderThanMinutes]);
        let released = 0;
        for (const { id } of candidates) {
            try {
                await database_1.db.transaction(async (conn) => {
                    ...
                    const [rows] = await conn.execute("SELECT id FROM orders WHERE id = ? AND status = 'pending' AND payment_status = 'pending' FOR UPDATE", [id]);
                    if (!rows[0])
                        return;
                ...
                logger_1.logger.error(`[OrderService] Failed to release abandoned order ${id}:`, err);
            }
        }
        if (released > 0) {
            logger_1.logger.info(`[OrderService] Released stock for ${released} abandoned online checkout(s)`);
        }
        return released;
    }
    async releaseReservedStockAndDeleteOrder(conn, orderId) {
        const [items] = await conn.execute('SELECT * FROM order_items WHERE order_id = ?', [orderId]);
        for (const item of items) {
            if (item.variant_id) {
                await conn.execute('UPDATE product_variants SET stock_quantity = stock_quantity + ? WHERE id = ?', [item.quantity, item.variant_id]);
    ...
    async syncStatusWithShiprocket(orderId, currentDbStatus, srStatus) {
        const s = srStatus.toLowerCase();
        let mappedStatus = '';
        if (s.includes('delivered'))
            mappedStatus = 'delivered';
        else if (s.includes('out for delivery') || s.includes('out_for_delivery'))
            mappedStatus = 'out_for_delivery';
        else if (s.includes('in transit') || s.includes('transit') || s.includes('shipped'))
            mappedStatus = 'shipped';
        else if (s.includes('picked') || s.includes('pickup done') || s.includes('pickup scheduled'))
            mappedStatus = 'processing';
        else if (s.includes('cancelled') || s.includes('canceled'))
            mappedStatus = 'cancelled';
        else if (s.includes('returned') || s.includes('rto'))
            mappedStatus = 'returned';
        if (mappedStatus && mappedStatus !== currentDbStatus) {
            // Validate that mappedStatus exists in order_statuses
            const exists = await database_1.db.queryOne('SELECT id FROM order_statuses WHERE slug = ?', [mappedStatus]);
            if (exists) {
                await database_1.db.query('UPDATE orders SET status = ?, updated_at = NOW() WHERE id = ?', [mappedStatus, orderId]);
                try {
    ...
    async updateStatusHistory(historyId, data, changedBy) {
        const statuses = await database_1.db.query('SELECT slug FROM order_statuses');
        const validStatuses = statuses.map(s => s.slug);
        if (!validStatuses.includes(data.status))
            throw new error_middleware_1.AppError('Invalid order status', 400);
        const existing = await database_1.db.queryOne('SELECT id, order_id, status, note FROM order_status_history WHERE id = ?', [historyId]);
        if (!existing)
            throw new error_middleware_1.AppError('History entry not found', 404);
        const historyDate = data.created_at ? new Date(data.created_at) : new Date();
        await database_1.db.query('UPDATE order_status_history SET status = ?, note = ?, created_at = ? WHERE id = ?', [data.status, data.note || null, historyDate, historyId]);
    ...
    async deleteStatusHistory(historyId, changedBy) {
        const existing = await database_1.db.queryOne('SELECT id, order_id, status, note FROM order_status_history WHERE id = ?', [historyId]);
        if (!existing)
            throw new error_middleware_1.AppError('History entry not found', 404);
        await database_1.db.query('DELETE FROM order_status_history WHERE id = ?', [historyId]);
        (0, activity_logger_1.logActivity)({
            action: 'order_status_changed',
            userId: changedBy,
            module: 'orders',
    ...
    generateOrderNumber() {
        ...
        const random = (0, helpers_1.randomToken)(6);
